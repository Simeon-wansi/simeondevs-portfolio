...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Only setup filters on DOM ready, don't render projects yet
    // Projects will be rendered when the page becomes active
    setupProjectFilters();
    
    console.log('✅ Projects module initialized (waiting for page activation)');
});

// Initialize projects when page becomes active
function initializeProjectsOnPageShow() {
    console.log('🔄 Projects page activated, initializing...');
    
    // Check if projects page is actually active
    const projectsPage = document.getElementById('projects');
    if (!projectsPage || !projectsPage.classList.contains('active')) {
        console.log('⚠️  Projects page not active, waiting...');
        setTimeout(initializeProjectsOnPageShow, 100);
        return;
    }
    
    // Now it's safe to render projects
    renderAllProjects();
    updateProjectStats();
    
    console.log('✅ Projects initialized on active page');
}

// Make this function globally available
window.initializeProjectsOnPageShow = initializeProjectsOnPageShow;

// Legacy function for backward compatibility
function initializeProjects() {
    console.log('🔄 Legacy initializeProjects called...');
    
    // Always try to render projects when called
    renderAllProjects();
    updateProjectStats();
...
function renderProjects(projects, container) {
    ...
    if (!projects || projects.length === 0) {
        console.error('❌ No projects data provided for rendering');
        return;
    }
    ...
    console.log(`✅ Projects rendered successfully - ${container.children.length} cards in container`);
}
...
// Export as SimeonDevProjects object for main.js compatibility
...
window.debugProjects = function() {
    console.log('🔍 Debug Projects Status:');
    console.log('- Projects data length:', projectsData.length);
    console.log('- Container element:', document.getElementById('allProjectsGrid'));
    console.log('- Container children:', document.getElementById('allProjectsGrid')?.children.length);
    console.log('- Projects page active:', document.getElementById('projects')?.classList.contains('active'));
